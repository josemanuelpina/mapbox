const isMobileMedia = window.matchMedia && window.matchMedia('(max-width: 600px)').matches;
document.addEventListener("DOMContentLoaded", function () {

    const infoTooltip = document.querySelector('.in_sidebar');
    const in_breadcrumb = document.querySelector('.in_breadcrumb_mobile');
    let startY;
    ...
    function handleTouchMove(e) {
        ...
        if (Math.abs(change) > 30) { // Umbral de movimiento para activar la animación
            // Swipe hacia arriba
            if (change > 0) {
                infoTooltip.style.height = '38rem';
                in_breadcrumb.classList.add('mobile_top');
                // infoTooltip.style.opacity = '1'
            }
            // Swipe hacia abajo
            else {
                infoTooltip.style.height = '19rem';
                in_breadcrumb.classList.remove('mobile_top');
                // infoTooltip.style.opacity = '.8'
            }
        }
    }
    if (isMobileMedia) {
    ...
    d3.csv('https://raw.githubusercontent.com/datosrtve/opendata/main/2023/distintivos-ambientales-dgt/vehiculos-es-wide.csv').then(data => {
        ...
        const vehiculosConfig = {
            ...
            otros: {
                source: '2023-09-AMBIENTALES-OTROS-96kuaj',
                url: 'mapbox://datosrtve.0gf70nrd',
                layer: 'vehiculosotroswide',
                distint: ['SIN', 'B', 'C', 'ECO', 'CERO']
            }
        }
        ...
        */
        ...
        const colorSchemeMapping = {
            'SIN': ['#e5e5e5', '#cccccc', '#b2b2b2', '#999999', '#808080', '#707070', '#4d4d4d'],
            'B': ['#fffbcc', '#fff89d', '#ffef5c', '#ffe000', '#efcc3a', '#e2bc00', '#d1a900'],
            'C': ['#e9f2e2', '#d7f2c5', '#b8e09d', '#a2db77', '#96ce65', '#8bc156', '#76b533'],
            'ECO': ['#d3edec', '#a6e8e8', '#8fcecf', '#63c6c6', '#3cbab3', '#34b2af', '#28a09a'],
            'CERO': ['#add7eb', '#8abfdb', '#66a7cc', '#5ba0c7', '#0077af', '#035376', '#0b3042'],
        };
        /*
        ...
        */
        ...
        document.querySelectorAll('.menu-buttonDistintivo').forEach((button) => {
            button.addEventListener('click', (event) => {
                ...
                document.querySelectorAll('.menu-buttonDistintivo').forEach((otherButton) => {
                ...
                if (datosVehiculos) {
                    const totalVehiculos = calcularTotalVehiculos(datosVehiculos, seleccionTipoVehiculo, distintivoActual);
                    document.getElementById('tooltip-tot_vehi').innerHTML = totalVehiculos.toLocaleString() + ' ';
                }
            });
        });
        ...
        var map = new mapboxgl.Map({
            ...
            // Deshabilita la rotación y el zoom con la rueda del ratón
            // scrollZoom: true,
            // Limita las interacciones a gestos de dos dedos
            // touchZoomRotate: true,
            // dragRotate: !isMobileMedia ? true : false,
            // dragPan: !isMobileMedia ? true : false,
        });
        ...
        const canariasBounds = [
            ...
            [-11.83017938181402, 34.43035740227728] //noroeste
        ];
        ...
        map.on('load', function () {
            ...
            document.querySelectorAll('.btn-vuelos').forEach((ele) => {
                ele.addEventListener('click', () => {
                    if (ele.getAttribute('id') === 'canarias') {
                        map.flyTo({
                ...
                for (let distintivo of config.distint) {
                    ...
                    if (!isMobileMedia) {
                        map.on('mouseleave', `layer-distintivo${distintivo}-${config.layer}`, function (e) {
                            ...
                            document.getElementById('tooltip-provincia').innerHTML = 'Parque Móvil en 2022'; // Vaciamos la provincia porque no es necesaria
                            document.getElementById('tooltip-poblacion').innerHTML = formatPoblacionTotal + ' '; //tooltip-tot-vehi
                            controlMouseLeave(e);
        ...
        window.updateMap = (tipoVehiculo) => {
            const botonesMenu = document.querySelector('.menu-content').querySelectorAll('.menu-button')
            botonesMenu.forEach((tipo) => {
                tipo.classList.remove('active')
            })
            document.querySelector('.menu-content').querySelector('.' + tipoVehiculo).classList.add('active')
        ...
        //Función para obtener los datos del vehículo seleccionado por el usuario y actaulizar `propsGlobales`
        ...
        function obtenerDatosVehiculo(tipoVehiculo, callback) {
            ...
            if (!map.getSource(configVehiculo.source) || !map.isSourceLoaded(configVehiculo.source)) {
                // console.error('La fuente no está cargada o no existe:', configVehiculo.source);
                return;
            }
        ...
        */
        ...
        function controlMouseMove(e) {
            ...
            if (e.features && e.features.length > 0 && e.features[0].source === hoveredSource) {
                ...
                if (props && tiposAreas.includes(props.tipo_area)) {
                    ...
                    document.getElementById('tooltip-poblacion').innerHTML = (props.pob ? Number(props.pob).toLocaleString('es-ES') + ' ' : 'N/A');
        ...
        function calcularTotalVehiculos(datosVehiculos, seleccionTipoVehiculo, distintivoActual) {
            ...
            let tipoVehiculoCSV = seleccionTipoVehiculo;
            // Campo del CSV distinto para autobuses
            if (seleccionTipoVehiculo === 'autobuses') {
                tipoVehiculoCSV = 'autobuses_otros';
            }
            ...
            if (seleccionTipoVehiculo === 'todos') {
                ...
                datosVehiculos.forEach(vehiculo => {
                    if (vehiculo.tipo_vehiculo === tipoVehiculoCSV) {
        ...
        // Valores máximos de cada distintivo
        const maxValuesByDistintivo = {
            'SIN': 50,
            'B': 25,
            'C': 25,
            'ECO': 20,
        ...
        function tooltipEdadMedia(edadMedia, edadMediaNacional, distintivo) {
            ...
            const width = container.node().getBoundingClientRect().width * 0.9;
            container.style("opacity", "1");
            const height = 5;
            const maxScaleValue = maxValuesByDistintivo[distintivo];
            ...
            const scaleBar = container.append('div')
                ...
                .style('height', '0px')
                .style('border', '0px solid #555555')
                .style('margin-left', 'auto')
                .style('margin-right', 'auto')
                .style('position', 'relative');

            // Añade las marcas y etiquetas de la escala
            [0, maxScaleValue].forEach(value => {
                scaleBar.append('div')
                    .style('width', '1px')
                    .style('height', '5px')
            ...
            scaleBar.append('div')
                .text(edadMedia.toFixed(1).replace('.', ',')) // Dos decimales
                .style('position', 'absolute')
            ...
            scaleBar.append('div')
                .text(edadMediaNacional.toFixed(1).replace('.', ','))
                .style('position', 'absolute')
        ...
        function tooltipEdadMediaDefault(edadMedia, edadMediaNacional, distintivo) {
            ...
            // Coloca el valor de la edad media
            scaleBar.append('div')
                .text(edadMediaNacional ? edadMediaNacional.toFixed(1).replace('.', ',') : edadMediaNacional)
                .style('position', 'absolute')
        ...
        function tooltipBarPorcentajes(porcentajes, colors, numTotal) {
            ...
            // Preparación contenedor para calcular el ancho
            requestAnimationFrame(() => {
                let containerWidth = container.node().getBoundingClientRect().width;
                let sumaPorcentajes = Object.values(porcentajes).reduce((a, b) => a + b, 0);
                let scaleFactor = sumaPorcentajes > 100 ? 100 / sumaPorcentajes : 1;
                //const color = colorDispositivo[distintivo];

                for (let distintivo in porcentajes) {
                    let porcentaje = porcentajes[distintivo] * scaleFactor;
                    let barWidth = containerWidth * (porcentaje / 100);
                    ...
                    // Tamaño del texto
                    let text = `${porcentaje.toFixed(1).replace('.', ',')}%`;
        ...
        */
        ...
        function barPorcentajesDefault(colors) {
            ...
            requestAnimationFrame(() => {
                ...
                for (let distintivo in porcentajes) {
                    let porcentaje = porcentajes[distintivo] * scaleFactor;
                    let barWidth = containerWidth * (porcentaje / 101);
        ...
        function actualizaLegenda(distintivo) {
            ...
            const paddingLegend = 10; // Espacio adicional a cada lado
            const anchoTotal = window.innerWidth * 0.25;
            const width = anchoTotal - paddingLegend * 2;
            const height = 20;

            const svg = legendContainer.append('svg')
                .attr('width', anchoTotal)
                .attr('height', height + 30); // Aumenta la altura para el texto

            // Gradiente
            const gradient = svg.append('defs')
                .append('linearGradient')
                .attr('id', 'gradient-leyenda')
                .attr('x1', '0%')
                .attr('x2', '100%')
                .attr('y1', '0%')
                .attr('y2', '0%');
            ...
            colorScheme.forEach((color, index) => {
                gradient.append('stop')
                    .attr('offset', `${index / (colorScheme.length - 1) * 100}%`)
                    .attr('stop-color', color);
            });

            // Rectángulo
            svg.append('rect')
                .attr('x', paddingLegend) // Comenzar después del padding
                .attr('y', 0)
                .attr('width', width)
                .attr('height', height)
                .style('fill', 'url(#gradient-leyenda)');

            // Etiquetas de texto
            const scaleValues = limitesEscala(distintivo).split(", ");
            scaleValues.forEach((value, index) => {
                let xPos = paddingLegend + (index / (scaleValues.length - 1)) * width; // Ajustar la posición x con padding
                // Ajusta la posición x para el primer y último texto
                if (index === 0) xPos = paddingLegend;
                if (index === scaleValues.length - 1) xPos = anchoTotal - paddingLegend;

                svg.append('text')
                    .attr('x', xPos)
                    .attr('y', height + 20) // Posición debajo del rectángulo
        ...
        function tooltipTabla(props) {
            ...
            rows.exit().remove();
            rows = rows.enter().append("tr").merge(rows).style("background-color", tipo => colorDispositivo[tipo]);

            // celdas a las filas
            rows.selectAll("td")
                .data(tipo => {
                    return [
                        { type: "text", value: tipo },
        ...
        function tooltipTabladefault(props, tipo_vehiculo) {
            ...
            let headerRow = thead.selectAll("tr").data([headers]);
            headerRow = headerRow.enter().append("tr").merge(headerRow)
                .style("background-color", "#ffffff"); // Fondo blanco para el encabezado
            ...
            rows = rows.enter().append("tr").merge(rows)
                .style("background-color", distintivoObj => colorDispositivo[distintivoObj.abbr]); // Aplica color a toda la fila

            // celdas a las filas
            rows.selectAll("td")
                .data(distintivoObj => {
                    return [
                        { type: "text", value: distintivoObj.abbr },
        ...
        const distintivoRadio = document.querySelectorAll('.menu-buttonDistintivo');
        const vehiculoTipoSelect = document.querySelector('.tipoVehiculo');
        const areaTipoCheckbox = document.querySelectorAll('.menu-button.areas');
        const in_breadcrumb_desktp = document.querySelector('.in_breadcrumb_desktp');
        const in_breadcrumb_mobile = document.querySelector('.in_breadcrumb_mobile');
        // console.log(distintivoRadio);
        ...
        function updateBreadcrumb() {
            ...
            distintivoRadio.forEach((ele) => {
                if (ele.classList.contains("active")) distintivo = ele.getAttribute('data-distintivo')
            })

            // Obtener el tipo de vehículo seleccionado
            tipoVehiculo = seleccionTipoVehiculo;

            // Obtener los tipos de área seleccionados

            let areaTipoStr = '';
            tipoArea = Array.from(areaTipoCheckbox).filter(ele => ele.classList.contains('active'));
            // console.log(tipoArea);
            if (tipoArea.length === 3) {
                areaTipoStr = 'Todas las áreas';
            } else if (tipoArea.length === 2) {
                areaTipoStr = `${tipoArea[0].value} + ${tipoArea[1].value}`;
            } else if (tipoArea.length === 1) {
                areaTipoStr = tipoArea[0].value;
            } else {
                areaTipoStr = 'Ninguna área seleccionada';
            }
            // Formatear la cadena para los tipos de área

            const renderHTML = (tipoVehiculo, distintivo, areaTipoStr) => {
                return `
                    <div class="in_icon"><span class="icon_breadcrumb in_icon-${tipoVehiculo}"></span><span>${tipoVehiculo}</span></div>
                    <div class="in_icon"><span class="icon_breadcrumb in_icon-etiqueta in_icon-${distintivo}"></span><span>${distintivo}</span></div>
                    <div class="in_icon"><span class="icon_breadcrumb in_icon-${areaTipoStr}"></span><span>${areaTipoStr}</span></div>
                `
            }
            // Actualizar la "miga de pan"
            // breadcrumb.textContent = `${tipoVehiculo}  >${distintivo} > ${areaTipoStr}`;
            //breadcrumb.textContent = `Distintivo ${distintivo} / ${tipoVehiculo} /  ${areaTipoStr}`;
            if (isMobileMedia) {
                in_breadcrumb_mobile.innerHTML = renderHTML(seleccionTipoVehiculo, distintivo, areaTipoStr);
            } else {
                in_breadcrumb_desktp.innerHTML = renderHTML(seleccionTipoVehiculo, distintivo, areaTipoStr);
            }
            // in_breadcrumb_desktp.innerHTML = renderHTML(seleccionTipoVehiculo, distintivo, areaTipoStr);
        ...
        document.querySelector('.in_close-menu').addEventListener('click', () => {
            document.querySelector('.in__footer_sidebar').classList.remove('active');
            document.querySelector('.in_overlight').classList.toggle('active');
        })

        document.querySelector('.in_overlight').addEventListener('click', () => {
            document.querySelector('.in__footer_sidebar').classList.remove('active');
            document.querySelector('.in_overlight').classList.toggle('active');
        })


        document.querySelectorAll(".areas").forEach((ele) => {
    ...
    document.querySelector(".in_edit_wrapper").addEventListener('click', () => {
        document.querySelector('.in__footer_sidebar').classList.toggle('active');
        document.querySelector('.in_overlight').classList.toggle('active');
    })
    document.querySelector(".in_breadcrumb_mobile").addEventListener('click', () => {
        document.querySelector('.in__footer_sidebar').classList.toggle('active');
        document.querySelector('.in_overlight').classList.toggle('active');
    })

    // document.querySelector("#close-onboard").addEventListener('click', () => {
    //     document.querySelector('.in_overlight-onboar').classList.add('in-pop-remove');
    // })
